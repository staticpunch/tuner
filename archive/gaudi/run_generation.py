...
def main():
    ...
    if args.dataset_name is None:
        ...
        ## NOTE: uncomment wrong loading dataset
        ...
        #     split = "train"
        # raw_dataset = (
        #     raw_dataset[split].shuffle().select(range(args.dataset_max_samples if args.dataset_max_samples > 0 else (raw_dataset[split]).num_rows))
        # )
        ## END NOTE
        ...
        def generate_dataset(batch):
            ...
            return prompt, outputs["sequences"].cpu()
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            if rank in [-1, 0]:
                ...
                # )
                answers = tokenizer.batch_decode(outputs, skip_special_tokens=True)[:args.batch_size*args.num_return_sequences]
                answers = [x.split(RESPONSE_KEY)[-1].strip() for x in answers]
                all_answers.extend(answers)
                print(f"Output: {answers}")
                print(separator)
        t_end = time.time()
        logger.info(f"Saving generated texts to {args.output_file}")
        with open(f"{args.output_file}", "w") as file:
            for i, answer in enumerate(all_answers):
                d = json.dumps({"quest_id": i, "answer_predict": answer}, ensure_ascii=False) + "\n"
                file.write(d)
        throughput = total_new_tokens_generated / duration
        # Print Stats
        if rank in [-1, 0]:
